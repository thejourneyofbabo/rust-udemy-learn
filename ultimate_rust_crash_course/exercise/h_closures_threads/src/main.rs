...
fn main() {
    ...
    let challenge_a = thread::spawn(move || {
        println!("Child thread 1 started");
        for value in rx2 {
            println!("Child thread 1 received: {}", value);
            //pause_ms(100);
        }
        println!("Child thread 1 finished");
    });

    let challenge_b = thread::spawn(move || {
        println!("Child thread 2 started");
        for value in rx {
            println!("Child thread 2 received: {}", value);
            //pause_ms(100);
        }
        println!("Child thread 2 finished");
    });

    // Uncomment and modify this part to send values
    let values = vec![1, 2, 3, 4, 5];
    for value in values {
        println!("Main thread: Sending {}", value);
        tx.send(value).unwrap();
        pause_ms(200); // Add a small pause to ensure some interleaving
    }
    //drop(tx); // Close the sending side of the channel

    challenge_a.join().unwrap();
    challenge_b.join().unwrap();
